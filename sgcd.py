...
class SGCD(GeneralRecommender):
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(SGCD, self).__init__(config, dataset)

        self.cl_rate = float(config['lambda'])
        self.eps = config['eps']
        self.n_layer = config['n_layer']
        self.layer_cl = config['l']
    ...
    def loss_function(self, y, t, drop_rate):
        # prediction y  label t
    ...
    def InfoNCE(self, view1, view2, temperature):
        ...
        logits = torch.cat([pos_score, ttl_score], dim=1)
        logits /= temperature
        labels = torch.zeros(logits.shape[0], dtype=torch.long)
        cl_loss = self.loss_function(logits, labels, self.drop)
        return cl_loss
    ...
    def forward(self, perturbed=False):
        ...
        embeddings_list = []
        all_embeddings_cl = all_embeddings
        for layer_idx in range(max(self.n_layers, self.hyper_layers * 2)):
            ...
            if layer_idx == self.layer_cl - 1:
                all_embeddings_cl = all_embeddings
        lightgcn_all_embeddings = torch.stack(embeddings_list, dim=1)  # [:self.n_layers + 1]
        lightgcn_all_embeddings = torch.mean(lightgcn_all_embeddings, dim=1)
        user_all_embeddings_cl, item_all_embeddings_cl = torch.split(all_embeddings_cl, [self.n_users,
                                                                                         self.n_items])
        user_all_embeddings, item_all_embeddings = torch.split(lightgcn_all_embeddings, [self.n_users, self.n_items])
        if perturbed:
            return user_all_embeddings, item_all_embeddings, user_all_embeddings_cl, item_all_embeddings_cl, embeddings_list
    ...
    def calculate_loss(self, interaction):
        ...
        user_all_embeddings, item_all_embeddings, user_all_embeddings_cl, item_all_embeddings_cl, embeddings_list = self.forward(
            perturbed=True)

        proto_loss = self.ProtoNCE_loss(self.get_ego_embeddings(), user, pos_item)
        ...
        sim_loss = self.cal_cl_loss(user, pos_item, user_all_embeddings, item_all_embeddings, user_all_embeddings_cl,
                                    item_all_embeddings_cl) * self.cl_rate


        reg_loss = self.reg_loss(u_embeddings, pos_embeddings, neg_embeddings)

        return mf_loss + self.reg_weight * reg_loss, sim_loss, proto_loss  # sim_loss  ,sim_loss,



    def cal_cl_loss(self, user, pos_item, user_view_1, item_view_1, user_view_2, item_view_2):
        u_idx = user
        i_idx = pos_item
    ...
    def predict(self, interaction):
        ...
        user_all_embeddings, item_all_embeddings, _, _, _ = self.forward(perturbed=True)
    ...
    def full_sort_predict(self, interaction):
        ...
        if self.restore_user_e is None or self.restore_item_e is None:
            self.restore_user_e, self.restore_item_e, _, _, _ = self.forward(perturbed=True)
